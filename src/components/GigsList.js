    ...
    <div className="relative w-[250px] h-[270px] bg-white rounded-lg shadow-md p-3 flex flex-col justify-between">
      ...
      <div className="flex items-center mb-2">
        <div className="w-[80px] h-[80px] relative mt-[50px] mr-3">
          <img src={companyLogo} alt="Company Logo" style={{ width: '60px', height: '60px', objectFit: 'contain' }} />
        </div>
      ...
      <div className="flex flex-col text-gray-500 text-sm mb-2">
        <div className="flex items-center ml-[10px] mb-3">
          <img src="/assets/briefcase.svg" alt="Company" style={{ width: '14px', height: '14px' }} className="mr-1" />
          <span>{companyName}</span>
        </div>
        <div className="flex items-center mt-[-30px] ml-[120px]">
          <img src="/assets/calender.svg" alt="Duration" style={{ width: '14px', height: '14px' }} className="mr-1" />
          <span>{duration}</span>
        </div>
        <div className="flex items-center ml-2">
          <img src="/assets/money.svg" alt="Rate" style={{ width: '14px', height: '14px' }} className="mr-1" />
          <span>{rate}</span>
        </div>
        <div className="flex items-center mt-[-18px] ml-[120px]">
          <img src="/assets/profile-fill.svg" alt="Rate" style={{ width: '14px', height: '14px' }} className="mr-1" />
          <span>{rate}</span>
        </div>
      </div>
      <Link to={`/gigdetail/${id}`} className="bg-[#263238] text-white px-4 py-2 rounded-md text-sm self-center text-center">
        Apply now
      </Link>
    </div>
  );
